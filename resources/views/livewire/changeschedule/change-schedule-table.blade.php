    ...
    <div class="p-4 rounded-lg dark:border-gray-700 ">
        <nav class="flex mb-4" aria-label="Breadcrumb">
            ...
            <li>
                ...
                <a href="{{route('ChangeScheduleTable')}}" class="ms-1 text-sm font-semibold text-gray-900 hover:text-blue-600 md:ms-2 dark:text-gray-400 dark:hover:text-white">Schedule Change</a>
                </div>
            </li>
        ...
        <h2 class="mb-4 text-3xl font-bold leading-none tracking-tight text-gray-900 md:text-3xl dark:text-white">Schedule Change</h2>

        
        <div class="flex justify-end">
            <button type="button" onclick="location.href='{{ route('ChangeScheduleForm') }}'" class="text-white mb-8 bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 me-2 mb-2 dark:bg-blue-600 dark:hover:bg-blue-700 focus:outline-none dark:focus:ring-blue-800">Create Schedule Change Request</button>
                ...
                <div>
                    <div>
                        ...
                        @else
                            ...
                            @php
                                $ctr = 0;
                                $pageIndex = ($ChangeScheduleData->currentpage() - 1) * $ChangeScheduleData->perpage() + $ctr;

                            @endphp
                            @foreach ($ChangeScheduleData as $index => $changeschedule)
                            @php
                                $ctr = $ctr + 1;
                            ...
                            @endphp
                                <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                                    ...
                                    @if($changeschedule->status == "Pending")
                                    <th scope="row" class="px-6 py-4 text-center font-medium text-gray-900 whitespace-nowrap dark:text-white capitalize">
                                        <span  class="text-gray-200 text-xs bg-amber-700 hover:bg-amber-800 focus:ring-4 focus:outline-none focus:ring-amber-300 font-medium rounded-lg  px-2 py-1 text-center inline-flex items-center me-2 dark:bg-amber-300 dark:hover:bg-amber-600 dark:focus:ring-amber-800">
                                            ...
                                            {{ $changeschedule->status }}
                                        </span>
                                    </th>
                                @elseif($changeschedule->status == "Approved")
                                    <th scope="row" class="px-6 py-4 text-center font-medium text-gray-900 whitespace-nowrap dark:text-white capitalize">
                                ...
                                @endif
                                    <td class="px-6 py-4 text-center whitespace-nowrap">
                                        {{$changeschedule->date_of_filling}}
                                    </td>
                                    <td class="px-6 py-4 text-left whitespace-nowrap">
                                        @php
                                            $changeschedule->original = json_decode($changeschedule->original);
                                        @endphp
                                        @foreach ($changeschedule->original as $original)
                                        <div class="mt-2">
                                            <li>
                                                <span class="textindigo">Time Schedule:</span>
                                               {{str_pad($original->start_time_schedule, 40,' ', STR_PAD_BOTH)}} - {{str_pad($original->end_time_schedule, 40,' ', STR_PAD_BOTH)}} 
                                            </li>
                                            <li>
                                                <span class="textindigo">Work Days:</span>
                                                @php
                                                    $original->work_days = $original->work_days ?? [];
                                                @endphp
                                                @foreach ($original->work_days as $day)
                                                    {{$day}}
                                                @endforeach
                                                {{-- {{str_pad($original->work_days, 40,' ', STR_PAD_BOTH)}}  --}}
                                             </li>
                                             <li>
                                                <span class="textindigo">Break Time:</span> {{str_pad($original->start_break_time, 40,' ', STR_PAD_BOTH)}} - {{str_pad($original->end_break_time, 40,' ', STR_PAD_BOTH)}}
                                             </li>
                                             <li>
                                               <span class="textindigo">Day Off:</span> {{str_pad($original->day_off, 40,' ', STR_PAD_BOTH)}}
                                             </li>
                                        </div>  
                                        @endforeach
                                    ...
                                    <td class="px-6 py-4 text-left whitespace-nowrap">
                                        @php
                                            $changeschedule->proposed = json_decode($changeschedule->proposed);
                                        @endphp
                                        @foreach ($changeschedule->proposed as $proposed)
                                        <div class="mt-2">
                                            <li>
                                                <span class="textindigo">Time Schedule:</span>
                                               {{str_pad($proposed->start_time_schedule, 40,' ', STR_PAD_BOTH)}} - {{str_pad($proposed->end_time_schedule, 40,' ', STR_PAD_BOTH)}} 
                                            </li>
                                            <li>
                                                <span class="textindigo">Work Days:</span>
                                                @php
                                                    $proposed->work_days = $proposed->work_days ?? [];
                                                @endphp
                                                @foreach ($proposed->work_days as $day)
                                                    {{$day}}
                                                @endforeach
                                                {{-- {{str_pad($proposed->work_days, 40,' ', STR_PAD_BOTH)}}  --}}
                                             </li>
                                             <li>
                                                <span class="textindigo">Break Time:</span> {{str_pad($proposed->start_break_time, 40,' ', STR_PAD_BOTH)}} - {{str_pad($proposed->end_break_time, 40,' ', STR_PAD_BOTH)}}
                                             </li>
                                             <li>
                                               <span class="textindigo">Day Off:</span> {{str_pad($proposed->day_off, 40,' ', STR_PAD_BOTH)}}
                                             </li>
                                        </div>  
                                        @endforeach
                                    </td>
                                    <td class="px-6 py-4 text-center">
                                        {{$changeschedule->reason}}
                                    </td>
                                    {{-- <a onclick="location.href='{{ route('ipcredit', ['index' => $changeschedule->id]) }}'"  class="cursor-pointer font-medium text-blue-600 dark:text-blue-500 hover:underline">Edit</a> --}}
                                        {{-- <a href="{{route('ipcredit', $changeschedule)}}" class="font-medium text-blue-600 dark:text-blue-500 hover:underline">Edit</a> --}}
                                        {{-- <a wire:click="removeIpcr({{$changeschedule->id}})" class="cursor-pointer font-medium text-red-600 dark:text-red-500 hover:underline ms-3">Remove</a> --}}
                                        ...
                                        <td class="items-center text-center py-4">
                                            ...
                                            <div class="hidden  top-0 right-0 mt-2 z-10 bg-white divide-y divide-gray-100 rounded-lg shadow w-44 dark:bg-gray-700" id="dropdown{{$loop->index}}">
                                                ...
                                                <ul class="py-2 text-sm text-gray-700 dark:text-gray-200">
                                                    {{-- @if (is_null($changeschedule->permit_to_teach)) --}}
                                                        <li>
                                                            <a onclick="location.href='{{ route('ChangeScheduleEdit', ['index' => $changeschedule->reference_num]) }}'"  class="block px-4 py-2 hover:bg-gray-100 dark:hover:bg-gray-600 dark:hover:text-white">Edit</a>
                                                        </li>
                                                    {{-- @endif --}}
                                                    {{-- @if ($changeschedule->permit_to_teach)
                                                    <li>
                                                        <a target="_blank" href="{{route('downloadTeachPermit', [ 'index' => $changeschedule->reference_num])}}" class="block px-4 py-2 hover:bg-gray-100 dark:hover:bg-gray-600 dark:hover:text-white">PDF</a>
                                                    </li>
                                                    @endif --}}

                                                </ul>
                                                <div class="py-2">
                                                    <a wire:click="removeScheduleChange('{{$changeschedule->reference_num}}')" wire:confirm="Are you sure you want to delete this post?" class="block px-4 py-2 text-black hover:bg-red-600 hover:text-white dark:hover:bg-gray-600 dark:hover:text-white">Delete</a>
                                                </div>
                                            </div>
                                        </td>
                                        ...
                                        {{-- <td class="items-center px-6 py-4">
                                            ...
                                            <div class="hidden bg-white divide-y divide-gray-100 rounded-lg shadow w-44 dark:bg-gray-700" id="dropdown{{$loop->index}}">
                                                ...
                                                <ul class="py-2 text-sm text-gray-700 dark:text-gray-200">
                                                    <li>
                                                        <a onclick="location.href='{{ route('TeachPermitEdit', ['index' => $changeschedule->id]) }}'"  class="block px-4 py-2 hover:bg-gray-100 dark:hover:bg-gray-600 dark:hover:text-white">Edit</a>
                                                    </li>
                                                    <li>
                                                        <a onclick="location.href='{{ route('TeachPermitPdf', ['index' => $changeschedule->id]) }}'" class="block px-4 py-2 hover:bg-gray-100 dark:hover:bg-gray-600 dark:hover:text-white">PDF</a>
                                                    </li>
                                                </ul>
                                                <div class="py-2">
                                                    <a wire:click="removeStudyPermit({{$changeschedule->id}})" wire:confirm="Are you sure you want to delete this post?" class="block px-4 py-2 text-black hover:bg-red-600 hover:text-white dark:hover:bg-gray-600 dark:hover:text-white">Delete</a>
                                                </div>
                                            </div>
                                        </td> --}}
                                </tr>
