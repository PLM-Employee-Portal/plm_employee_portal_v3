    ...
    <div class="p-4 rounded-lg dark:border-gray-700 ">
        <nav class="flex mb-4" aria-label="Breadcrumb">
            ...
            <li>
                ...
                <a href="{{route('CreditsMonetizationTable')}}" class="ms-1 text-sm font-semibold text-gray-900 hover:text-blue-600 md:ms-2 dark:text-gray-400 dark:hover:text-white">Credits Monetization</a>
                </div>
            </li>
        ...
        <h2 class="mb-4 text-3xl font-bold leading-none tracking-tight text-gray-900 md:text-3xl dark:text-white">Credits Monetization</h2>
            ...
            <table class="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400 pb-4">
                <thead class="text-xs text-gray-700 uppercase bg-gray-100 dark:bg-gray-700 dark:text-gray-400">
                    <tr>
                        ...
                        <th scope="col" class="px-6 py-3 text-center">
                            Vacation Credits Requested
                        </th>
                        <th scope="col" class="px-6 py-3 text-center">
                            Sick Credits Requested
                        </th>
                        <th scope="col" class="px-6 py-3 text-center">
                            Total Requested
                        </th>
                ...
                <div>
                    <div>
                        ...
                        @else
                            ...
                            @php
                                $ctr = 0;
                                $pageIndex = ($MonetizationData->currentpage() - 1) * $MonetizationData->perpage() + $ctr;

                            @endphp
                            @foreach ($MonetizationData as $index => $monetization)
                            @php
                                $ctr = $ctr + 1;
                            ...
                            @endphp
                                ...
                                @endif
                                    <td class="px-6 py-4 text-center whitespace-nowrap">
                                        {{$monetization->date_of_filling}}
                                    </td>
                                    <td class="px-6 py-4 text-center whitespace-nowrap">
                                        {{$monetization->requested_vacation_credits}}
                                    </td>
                                    <td class="px-6 py-4 text-center">
                                        {{$monetization->requested_sick_credits}}
                                    </td>
                                    <td class="px-6 py-4 text-center">
                                        {{$monetization->total_requested}}
                                    </td>
                                    <td class="px-6 py-4 text-center">
                                        {{$monetization->purpose}}
                                    </td>
                                    {{-- <a onclick="location.href='{{ route('ipcredit', ['index' => $monetization->id]) }}'"  class="cursor-pointer font-medium text-blue-600 dark:text-blue-500 hover:underline">Edit</a> --}}
                                        {{-- <a href="{{route('ipcredit', $monetization)}}" class="font-medium text-blue-600 dark:text-blue-500 hover:underline">Edit</a> --}}
                                        {{-- <a wire:click="removeIpcr({{$monetization->id}})" class="cursor-pointer font-medium text-red-600 dark:text-red-500 hover:underline ms-3">Remove</a> --}}
                                        ...
                                        <td class="items-center text-center py-4">
                                            ...
                                            <div class="hidden  top-0 right-0 mt-2 z-10 bg-white divide-y divide-gray-100 rounded-lg shadow w-44 dark:bg-gray-700" id="dropdown{{$loop->index}}">
                                                ...
                                                <ul class="py-2 text-sm text-gray-700 dark:text-gray-200">
                                                    @if (is_null($monetization->permit_to_teach))
                                                        <li>
                                                            <a onclick="location.href='{{ route('CreditsMonetizationEdit', ['index' => $monetization->reference_num]) }}'"  class="block px-4 py-2 hover:bg-gray-100 dark:hover:bg-gray-600 dark:hover:text-white">Edit</a>
                                                        </li>
                                                    @endif
                                                    {{-- @if ($monetization->permit_to_teach)
                                                    <li>
                                                        <a target="_blank" href="{{route('downloadTeachPermit', [ 'index' => $monetization->reference_num])}}" class="block px-4 py-2 hover:bg-gray-100 dark:hover:bg-gray-600 dark:hover:text-white">PDF</a>
                                                    </li>
                                                    @endif --}}

                                                </ul>
                                                <div class="py-2">
                                                    <a wire:click="removeTeachPermit('{{$monetization->reference_num}}')" wire:confirm="Are you sure you want to delete this post?" class="block px-4 py-2 text-black hover:bg-red-600 hover:text-white dark:hover:bg-gray-600 dark:hover:text-white">Delete</a>
                                                </div>
                                            </div>
                                        </td>
                                        ...
                                        {{-- <td class="items-center px-6 py-4">
                                            ...
                                            <div class="hidden bg-white divide-y divide-gray-100 rounded-lg shadow w-44 dark:bg-gray-700" id="dropdown{{$loop->index}}">
                                                ...
                                                <ul class="py-2 text-sm text-gray-700 dark:text-gray-200">
                                                    <li>
                                                        <a onclick="location.href='{{ route('TeachPermitEdit', ['index' => $monetization->id]) }}'"  class="block px-4 py-2 hover:bg-gray-100 dark:hover:bg-gray-600 dark:hover:text-white">Edit</a>
                                                    </li>
                                                    <li>
                                                        <a onclick="location.href='{{ route('TeachPermitPdf', ['index' => $monetization->id]) }}'" class="block px-4 py-2 hover:bg-gray-100 dark:hover:bg-gray-600 dark:hover:text-white">PDF</a>
                                                    </li>
                                                </ul>
                                                <div class="py-2">
                                                    <a wire:click="removeStudyPermit({{$monetization->id}})" wire:confirm="Are you sure you want to delete this post?" class="block px-4 py-2 text-black hover:bg-red-600 hover:text-white dark:hover:bg-gray-600 dark:hover:text-white">Delete</a>
                                                </div>
                                            </div>
                                        </td> --}}
                                </tr>
